-- Extract msgids passed to _() from the reading style plugin source files.
...
local function readString(src, i)
    ...
    elseif c == "[" then
        ...
        local body = src:sub(start, e - 1)
        -- Lua swallows a newline immediately after the opening long bracket, so
        -- the runtime msgid does not have it. Without this the extracted key is
        -- off by one character and never matches at runtime.
        body = body:gsub("^\r?\n", "")
        return body, e + #close
    end
    return nil
