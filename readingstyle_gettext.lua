--[[--
Self-contained localization for the Reading style plugin.
...
Usage is identical to gettext, so existing call sites need only swap the require:
    ...
    local label = _("Paragraph indentation")
...
local function loadLangTable(lang)
    ...
    local function tryCode(code)
        ...
        logger.warn("readingstyle_gettext: failed to load", path, tbl)
        return nil
    end
...
local ReadingStyleGetText = setmetatable({}, {
